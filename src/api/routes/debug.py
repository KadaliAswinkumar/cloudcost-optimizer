...
async def get_database_status(db: AsyncSession = Depends(get_db)):
    ...
    spot_pricing = await db.execute(
        select(func.count()).select_from(CloudPricing)
        .where(CloudPricing.pricing_type == "spot")
    )
    ...
    return {
        ...
        "diagnosis": {
            ...
            "issue": "No on-demand pricing in cloud_pricing table!" if on_demand_pricing.scalar() == 0 else None
        }
    }
